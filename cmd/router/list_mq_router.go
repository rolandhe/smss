...
func (r *mqListRouter) Router(conn net.Conn, commHeader *protocol.CommonHeader, worker MessageWorking) error {
	...
	if len(infos) == 0 {
		outBuff := make([]byte, nets.RespHeaderSize)
	...
	outBuff := make([]byte, len(jBuff)+nets.RespHeaderSize)
	binary.LittleEndian.PutUint16(outBuff, protocol.OkCode)
	binary.LittleEndian.PutUint32(outBuff[2:], uint32(len(jBuff)))
	copy(outBuff[nets.RespHeaderSize:], jBuff)
	return nets.WriteAll(conn, outBuff)
}
